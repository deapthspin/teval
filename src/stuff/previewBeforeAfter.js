...
const PreviewBeforeAfter = () => {
    ...
    let [canRenderValue, setCanRenderValue] = useState(false)
    ...
    async function submit() {
        ...
        setAnswers(answers = [])
        let res = await fetch('https://formcreationapppostgresql.herokuapp.com/forms')
        let data = await res.json()
            ...
            let response = await fetch(`https://formcreationapppostgresql.herokuapp.com/questions/${currentFormId}`)
            let data2 = await response.json()
                ...
                let res2 = await fetch(`https://formcreationapppostgresql.herokuapp.com/responses/${currentFormId}`)
                ...
                let res3 = await fetch(`https://formcreationapppostgresql.herokuapp.com/responses/${currentFormId + 1}`)
                ...
                let response1 = await fetch(`https://formcreationapppostgresql.herokuapp.com/questions/${currentFormId}`)
    ...
    function submitForm(e) {
        ...
        for(let i = 0; i < answers.length; i++) {
            ...
            fetch(`https://formcreationapppostgresql.herokuapp.com/forms/${currentFormId}/responses`, {
                method: "POST",
                headers: {
    ...
    function sendToForm(e) {
        setCanRender(false)
        setCanRenderValue(canRenderValue = false)
        ...
        setTimeout(() => {
            ...
            setCanRenderValue(canRenderValue = true)
    ...
    function deleteForm(e) {
        ...
        if(response) {
           fetch(`https://formcreationapppostgresql.herokuapp.com/forms/${currentFormId}`, {
            method: 'DELETE',
    ...
    function questionChange(e) {
        fetch(`https://formcreationapppostgresql.herokuapp.com/questions/${currentFormId}/${e.target.id}`, {
            method: "PUT",
            body: JSON.stringify({
            ...
            <section>
                ...
                {questions.length > 0 && <ol className='formNquestions'>
                    ...
                    {questions.length > 0 && <Paper className='formUidPaper'>
                        ...
                        {canRenderValue && <h1>Program value add: <br/> {canRenderValue && <span className='programvalueadd'>{!(((allAddedAfter - allAddedBefore) / allAddedBefore) * 100) ? '' : (((allAddedAfter - allAddedBefore) / allAddedBefore) * 100) >= 0 ? '+' : '' }{(((allAddedAfter - allAddedBefore) / allAddedBefore) * 100).toFixed(3)}%</span>}</h1>}
